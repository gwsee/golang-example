...
func NewMysql(c driver.MysqlConfig) (*gorm.DB, error) {
	mysqlMasterDriver := driver.New(driver.WithMysqlDsn(c.Write))
	instance, err := New(
		mysqlMasterDriver,
		&gorm.Config{
			Logger: logger.Default.LogMode(logger.Info),
		},
		WithMaxOpenConn(int(c.MaxOpenConn)),
		WithMaxIdleConn(int(c.MaxIdleConn)),
		WithConnMaxIdleTime(time.Duration(c.ConnMaxIdleTime)),
		WithConnMaxLifetime(time.Duration(c.ConnMaxLifetime)),
	)
	if err != nil {
		return nil, err
	}
